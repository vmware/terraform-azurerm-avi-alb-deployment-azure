...
| <a name="input_configure_vnet_peering"></a> [configure\_vnet\_peering](#input\_configure\_vnet\_peering) | This variable is used to peer the created VNET with another VNET | <pre>object({<br>    enabled        = bool,<br>    resource_group = string,<br>    vnet_name      = string,<br>    global_peering = bool<br>  })</pre> | <pre>{<br>  "enabled": false,<br>  "global_peering": true,<br>  "resource_group": "",<br>  "vnet_name": ""<br>}</pre> | no |
